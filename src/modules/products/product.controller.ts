...
const normalizeImagePath = (file?: Express.Multer.File) => {
  if (!file) return undefined;
  return `/uploads/products/${file.filename}`;
};

const createProduct: RequestHandler = catchAsync(async (req, res) => {
  const result = await ProductService.createProduct(req.body, normalizeImagePath(req.file));
...
const getProductById: RequestHandler = catchAsync(async (req, res) => {
  ...
  sendResponse(res, { statusCode: 200, success: true, message: 'Product retrieved successfully', data: result });
});

const updateProduct: RequestHandler = catchAsync(async (req, res) => {
  const result = await ProductService.updateProduct(req.params.id, req.body, normalizeImagePath(req.file));
